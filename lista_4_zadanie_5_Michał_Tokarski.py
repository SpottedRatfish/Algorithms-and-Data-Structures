...
def diff(Tree, variable):
    """Funkcja bazuje na rekurencji. Obsługuje mnożenie, dodawanie, odejmowanie i dzielenie.
    Potęgi zapisać w formie mnożenia, to jest x^2=x*x"""
    derr = BinaryTree()
...
def to_expression(Tree):
    ...
    if number_check(Tree[0]):
        ...
        if type(Tree[1]) in [int, float] and type(Tree[2]) in[int, float]:
            ...
            expr = f'{Tree[1]}{Tree[0]}2'
            result = str(eval(expr))
            result = result.replace('2', Tree[2])
            if eval(expr) < 0:
                result = '(' + result + ')'

        elif Tree[2] == 0:
            expr = f'2{Tree[0]}{Tree[2]}'
            result = str(eval(expr))
            result = result.replace('2', Tree[1])
            if eval(expr) < 0:
                result = '(' + result + ')'
